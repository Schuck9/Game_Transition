"""
A simple implementation of Ultimatum Game in complex network with game transition
@date: 2020.5.15
@author: Tingyu Mo
"""
...
class UG_Complex_Network():
    def __init__(self,node_num = 10000,network_type = "SF",update_rule ="NS",player_type = "B",
        ...
        self.avg_pq_list=[]
        self.frequency_matrix=np.zeros((5,5))
    ...
    def build_network(self,network_type = None):
        ...
        if network_type == "SF":
            ...
            G = nx.random_graphs.random_regular_graph(self.avg_degree, self.node_num,1) #seed == 1 ,98%
            G = self.network_weights_asign(G)
            G = self.network_game_asign(G)
    ...
    def network_game_asign(self,G):
        #博弈初始化
        for n in list(G.nodes()):
            nbrs = list(G.adj[n])
            for nbr in nbrs:
                # G.edges[n, nbr]['game'] = np.random.choice([0,1],size = 1)[0] #随机给定初始博弈
                G.edges[n, nbr]['game'] = 0 #给定高分配额的博弈
        print("Game initialzed!")
        return G
    ...
    def strategy_asigned(self,G,node_list,Type = 'B'):
        ...
        if Type == 'B':
            ...
            stg = [0,0.25,0.50,0.75,1]
            for n in node_list:
                #Type-D player     
                G.nodes[n]['p'] = np.random.choice(stg,size=1)[0]
                G.nodes[n]['q'] = np.random.choice(stg,size=1)[0]
                G.nodes[n]['payoff'] = 0 
    ...
    def synchronous_play(self,G):
        ...
        for n, nbrs in G.adjacency():
            for nbr, _ in nbrs.items():
                ...
                weight = G.edges[n, nbr]['weight'] 
                game = 0.5 if G.edges[n, nbr]['game'] else 1
                if offer > demand:
                    G.nodes[n]['payoff'] += (1-offer)*weight*game
                    # G.nodes[nbr]['payoff'] += offer
                    # G.edges[n, nbr]['game'] = 0
                else:
                    fairness = False
                ...
                if offer > demand:
                    ...
                    G.nodes[n]['payoff'] += offer*weight*game
                    # G.edges[n, nbr]['game'] = 0
                else:
                    fairness = False
                # fairness = True
                G.edges[n, nbr]['game'] = 0 if fairness else 1
    ...
    def death_birth_updating(self,G):
        ...
        # reproduce_probability = self.normalization(reproduce_probability)
    ...
    def mutation(self,G,chosen_individual,reproduce_individual):
        if np.random.rand(1) <= self.mutate_rate:
            if self.player_type == 'D':
                stg = [0,0.25,0.5,0.75,1]
                G.nodes[chosen_individual]['p'] = np.random.choice(stg,size=1)[0]
                G.nodes[chosen_individual]['q'] = np.random.choice(stg,size=1)[0]
            else:
    ...
    def frequency_calculate(self,G,Epoch):
        '''
        calculate discrete stratagy's frequency
        '''
        p_vector = self.get_all_values(G,'p')
        q_vector = self.get_all_values(G,'q')
        frequency_matrix = self.frequency_matrix*(Epoch-1)*self.node_num
        for i in range(0,self.node_num):
            p_index = int(p_vector[i]*4)
            q_index = int(q_vector[i]*4)
            # New_Frequency =   (self.frequency_matrix[p_index][q_index]*(Epoch-1) + 1)*1.0/Epoch
            frequency_matrix[p_index][q_index] += 1
        self.frequency_matrix =frequency_matrix*1.0/(Epoch*self.node_num)
    ...
    def save(self,G,Epoch):
        ...
        p_vector = self.get_all_values(G,'p')
        q_vector = self.get_all_values(G,'q')
        p_count = Counter(p_vector)
        q_count = Counter(q_vector)
        print("p:",p_count)
        print("q:",q_count)
        p_count_path = os.path.join(Epoch_dir,info+"_p_count.npy")
        q_count_path = os.path.join(Epoch_dir,info+"_q_count.npy")
        np.save(p_count_path, p_count) 
        np.save(q_count_path, q_count) 
        ...
        if self.player_type == 'D':
            frequency_path = os.path.join(Epoch_dir,info+"_frequency_matrix.csv")
...
if __name__ == '__main__':
    ...
    player_type = "D" # [A=(p=q,q), B=(p,1-p), C=(p,q)]
    avg_degree = 4
    ...
    for Epoch in range(Start,Epochs+1):
        ...
        UG.avg_strategy = UG.avg_strategy_calculate(G,Epoch)
        if UG.player_type =='D':
            UG.frequency_calculate(G,Epoch)
        if Epoch % 1000 == 0:
            print("Epoch[{}]".format(Epoch))
            print("Average strategy: (p ,q)={}\n".format(UG.avg_strategy))
            UG.avg_pq_list.append(UG.avg_strategy)
